		...
		for (int i = 1; i < cols.length; i++) {
			if (cols[i].startsWith("Goto")) {
				...
				try {
					...
					ErrorHandler.printError("something wrong...");
				}
			} else {
			...
			for (int j = 1; j < cols.length; j++) {
				if (!cols[j].equals("")) {
					if (cols[j].equals("acc")) {
						actionTable.get(actionTable.size() - 1).put(terminals.get(j), Action.buildAcceptAction());
					} else if (terminals.containsKey(j)) {
//                        try {
						Token t = terminals.get(j);
						Action a;
						int number = Integer.parseInt(cols[j].substring(1));
						if (cols[j].charAt(0) == 'r') {
							a = Action.buildReduceAction(number);
						} else {
							a = Action.buildShiftAction(number);
						}
